...
class LanguageServer {
	...
	**/
	...
	public static final DontShowOldPreviewHintAgainKey = new HaxeMementoKey<Bool>("dontShowRC3HintAgain");
	...
	function onDidDetectOldPreview(?data:{preview:String}) {
		...
		final message = 'Old Haxe 4 preview build detected'
			+ detectedVersion
			+ '. Consider upgrading to Haxe 4.0.0-rc.3 for improved completion features and stability.';
		window.showInformationMessage(message, VisitDownloadPageOption, DontShowAgainOption).then(function(selection) {
			switch selection {
				...
				case VisitDownloadPageOption:
					env.openExternal(Uri.parse("https://haxe.org/download/version/4.0.0-rc.3/"));
